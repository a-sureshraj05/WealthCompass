...
const HoldingsTable: React.FC<Props> = ({ holdings, onRemove }) => {
      ...
      <table className="w-full text-left">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200">
            ...
            <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Current</th>
        ...
        <tbody className="divide-y divide-slate-200">
          {holdings.map((h) => (
            <tr key={h.id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="px-6 py-4 text-sm text-slate-700 font-medium">${h.avgPrice.toFixed(2)}</td>
              <td className="px-6 py-4 text-sm text-slate-700 font-medium">${h.currentPrice.toFixed(2)}</td>
              <td className="px-6 py-4">
                <div className="text-sm font-bold text-slate-900">${(h.quantity * h.currentPrice).toLocaleString()}</div>
                <div className={`text-xs ${(h.currentPrice - h.avgPrice) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
