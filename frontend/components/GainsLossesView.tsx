...
const GainsLossesView: React.FC<Props> = ({ realizedGains: realizedGainsData, unrealizedGains: unrealizedGainsData, selectedBrokerages, setSelectedBrokerages, selectedTickers, setSelectedTickers }) => {
  ...
  const SortIndicator = ({ column }: { column: GainSortKey }) => {
    ...
    return (
      <span className="ml-1 text-[#0052FF]">
        {sortDirection === 'asc' ? (
  ...
  const GainTableSection = ({ title, data }: { title: string, data: (RealizedGain | UnrealizedLot)[] }) => (
    <div className="space-y-3">
      ...
      {data.length === 0 ? (
        <div className="py-8 text-center bg-[#F9F9FB] border border-dashed border-[#E5E5E5] rounded-lg text-slate-400 text-xs">
          No entries in this category for the current filter.
        </div>
      ) : (
        <div className="overflow-hidden rounded-lg border border-[#E5E5E5] bg-white overflow-x-auto">
          <table className="w-full text-left min-w-[900px]">
            <thead>
              <tr className="bg-[#F3F3F5] border-b border-[#E5E5E5]">
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors" onClick={() => handleSort('ticker')}>
                  <div className="flex items-center">Ticker <SortIndicator column="ticker" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors" onClick={() => handleSort('assetType')}>
                  <div className="flex items-center">Asset Type <SortIndicator column="assetType" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors" onClick={() => handleSort('brokerage')}>
                  <div className="flex items-center">Brokerage <SortIndicator column="brokerage" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors" onClick={() => handleSort('buyDate')}>
                  <div className="flex items-center">Bought <SortIndicator column="buyDate" /></div>
                </th>
                {activeSubTab === 'realized' && (
                  <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors" onClick={() => handleSort('sellDate')}>
                    <div className="flex items-center">Sold <SortIndicator column="sellDate" /></div>
                  </th>
                )}
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors text-right" onClick={() => handleSort('quantity')}>
                  <div className="flex items-center justify-end">Qty <SortIndicator column="quantity" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors text-right" onClick={() => handleSort('buyPrice')}>
                  <div className="flex items-center justify-end">Cost/Sh <SortIndicator column="buyPrice" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors text-right" onClick={() => handleSort('price')}>
                  <div className="flex items-center justify-end">{activeSubTab === 'realized' ? 'Sold/Sh' : 'Mkt/Sh'} <SortIndicator column="price" /></div>
                </th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#0052FF] transition-colors text-right" onClick={() => handleSort('gain')}>
            ...
            <tbody className="divide-y divide-[#E5E5E5]">
              {data.map((g: RealizedGain | UnrealizedLot) => (
                <tr key={g.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tight ${
                      g.brokerage.toLowerCase().includes('robinhood') ? 'bg-[#0052FF] text-white' :
                      g.brokerage.toLowerCase().includes('schwab') ? 'bg-[#434656] text-white' :
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center p-1 bg-slate-100 rounded-2xl w-fit">
        <button
          ...
          className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
            activeSubTab === 'realized' ? 'bg-white text-[#0052FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          Realized
        </button>
        <button
          ...
          className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${
            activeSubTab === 'unrealized' ? 'bg-white text-[#0052FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          Unrealized
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-10 gap-6">
        {activeSubTab === 'realized' ? (
          <div className="md:col-span-7 bg-white p-6 rounded-lg border border-[#E5E5E5]">
        ...
        <div className="md:col-span-3 bg-white p-6 rounded-lg border border-[#E5E5E5] flex flex-col justify-center">
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                <label className="block text-[8px] font-black text-[#0052FF] uppercase mb-1">LT Rate %</label>
                <input
                  type="number"
                  value={ltTaxRate}
                  onChange={(e) => setLtTaxRate(Number(e.target.value))}
                  className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 focus:ring-2 focus:ring-[#0052FF] outline-none"
                />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
        {/* Filter Card */}
        <div className="lg:col-span-7 bg-white p-5 rounded-lg border border-[#E5E5E5] flex flex-col gap-4 z-30">
          ...
          <div className="flex flex-wrap items-center gap-4">
            {activeSubTab === 'realized' && (
              <div className="relative group">
                <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#0052FF] uppercase tracking-tighter">Tax Year</label>
                <select 
                  value={selectedYear}
                  onChange={(e) => setSelectedYear(e.target.value)}
                  className="appearance-none pl-3 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#0052FF] outline-none cursor-pointer min-w-[120px]"
                >
            ...
            <div className="relative" ref={brokerageMenuRef}>
              <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#0052FF] uppercase tracking-tighter z-10">Brokerage</label>
              <button
                onClick={() => setIsBrokerageMenuOpen(!isBrokerageMenuOpen)}
                className="flex items-center justify-between pl-3 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#0052FF] outline-none cursor-pointer min-w-[140px] text-left"
              >
              ...
              {isBrokerageMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedBrokerages.length > 0 && <button onClick={() => setSelectedBrokerages([])} className="text-[10px] font-bold text-[#0052FF] px-2">Clear</button>}
                  </div>
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {uniqueBrokerages.map(broker => (
                      <label key={broker} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#0052FF] focus:ring-[#0052FF]" checked={selectedBrokerages.includes(broker)} onChange={() => toggleBrokerage(broker)} />
            ...
            <div className="relative" ref={assetTypeMenuRef}>
              <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#0052FF] uppercase tracking-tighter z-10">Asset Type</label>
              <button
              ...
              {isAssetTypeMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedAssetTypes.length > 0 && <button onClick={() => setSelectedAssetTypes([])} className="text-[10px] font-bold text-[#0052FF] px-2">Clear</button>}
                  </div>
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {uniqueAssetTypes.map(type => (
                      <label key={type} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#0052FF] focus:ring-[#0052FF]" checked={selectedAssetTypes.includes(type)} onChange={() => toggleAssetType(type)} />
            ...
            <div className="relative" ref={tickerMenuRef}>
              <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#0052FF] uppercase tracking-tighter z-10">Ticker</label>
              <button
              ...
              {isTickerMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedTickers.length > 0 && <button onClick={() => setSelectedTickers([])} className="text-[10px] font-bold text-[#0052FF] px-2">Clear</button>}
                  </div>
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {uniqueTickers.map(ticker => (
                      <label key={ticker} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#0052FF] focus:ring-[#0052FF]" checked={selectedTickers.includes(ticker)} onChange={() => toggleTicker(ticker)} />
        ...
        {/* Total Summary Card */}
        <div className="lg:col-span-3 bg-[#0052FF] p-5 rounded-lg text-white flex items-center justify-between">
          <div>
            <div className="text-[10px] font-black text-[#B7C4FF] uppercase tracking-widest mb-1">
          ...
          <div className="w-10 h-10 bg-[#003EC7] rounded-lg flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="space-y-4">
          <button
            ...
            className="w-full p-4 bg-[#DDE1FF]/20 rounded-lg border border-[#C3C5D9] flex items-center justify-between hover:bg-[#DDE1FF]/40 transition-colors"
          >
            <div className="flex items-center gap-3">
              <svg className={`w-4 h-4 text-[#0052FF] transition-transform ${ltExpanded ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" />
              </svg>
              <div className="text-left">
                <h4 className="text-[11px] font-black text-[#003EC7] uppercase tracking-widest">Long-Term Summary</h4>
                <p className="text-[10px] text-[#0052FF] font-medium">{activeSubTab === 'realized' ? 'Closed positions' : 'Open lots'} held {'>'} 1 year · {longTerm.length} lots</p>
              </div>
            </div>
            <div className={`text-xl font-black ${stats.ltTotal >= 0 ? 'text-[#003EC7]' : 'text-[#BA1A1A]'}`}>
