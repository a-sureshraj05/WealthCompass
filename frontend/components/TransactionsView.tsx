      ...
      <div className="bg-white p-5 rounded-lg border border-[#D2D2D7] relative z-30">
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4 flex-1">
                ...
                <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedBrokerages.length > 0 && (
                      <button onClick={() => setSelectedBrokerages([])} className="text-[10px] font-bold text-[#007AFF] hover:text-[#0062CC] px-2">Clear</button>
                    )}
                  </div>
            ...
            <div className="relative" ref={assetTypeMenuRef}>
              ...
              <button
                ...
                className="flex items-center justify-between pl-3 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#007AFF] outline-none cursor-pointer min-w-[130px] text-left"
              >
                ...
                <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedAssetTypes.length > 0 && (
                      <button onClick={() => setSelectedAssetTypes([])} className="text-[10px] font-bold text-[#007AFF] hover:text-[#0062CC] px-2">Clear</button>
                    )}
                  </div>
                ...
                <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                  <div className="p-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    {selectedTickers.length > 0 && (
                      <button onClick={() => setSelectedTickers([])} className="text-[10px] font-bold text-[#007AFF] hover:text-[#0062CC] px-2">Clear</button>
                    )}
                  </div>
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
            ...
            <div className="flex items-center gap-2">
              <div className="relative group">
                <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#007AFF] uppercase tracking-tighter">Timeframe</label>
                <select 
                  value={dateRangeType}
                  onChange={(e) => setDateRangeType(e.target.value as DateRangeType)}
                  className="appearance-none pl-3 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#007AFF] outline-none cursor-pointer min-w-[150px]"
                >
              ...
              {dateRangeType === 'custom' && (
                <div className="flex items-center space-x-2 animate-in slide-in-from-left-2 duration-200">
                  <input 
                    ...
                    className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:ring-2 focus:ring-[#007AFF] outline-none"
                  />
          ...
          <div className="relative">
            <label className="absolute -top-2 left-2 bg-white px-1 text-[9px] font-black text-[#007AFF] uppercase tracking-tighter z-10">Show</label>
          ...
          <button
            ...
            className="text-xs font-bold text-slate-400 hover:text-[#007AFF] transition-colors uppercase tracking-widest px-2"
          >
            Reset
          </button>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#F5F5F7] border-b border-[#D2D2D7]">
                ...
                <th
                  className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#007AFF] transition-colors group/header"
                ...
                <th 
                  className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-[#007AFF] transition-colors group/header text-right"
            ...
            <tbody className="divide-y divide-[#D2D2D7]">
              {filteredTransactions.map((t) => {
                ...
                const inputCls = "w-full px-2 py-1 text-xs border border-[#007AFF] rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]";
                ...
                return (
                  <tr key={`${t.brokerage}-${t.id}`} className={`transition-colors group ${isEditing ? 'bg-[#007AFF]/5' : 'hover:bg-[#F5F5F7]'} ${t.is_deleted && !isEditing ? 'opacity-40' : ''}`}>
                    {/* Hide checkbox */}
                    ...
                    <td className="px-6 py-4">
                      {isEditing ? (
                        ...
                        <span className={`inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tight ${
                          t.brokerage.toLowerCase().includes('robinhood') ? 'bg-[#007AFF] text-white' :
                    ...
                    <td className="px-6 py-4 text-right">
                      {isEditing ? (
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={saveEdit} className="p-1.5 text-white bg-[#007AFF] hover:bg-[#0062CC] rounded-lg transition-all" title="Save">
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg transition-all ${
                              t.is_override
                                ...
                                : 'text-slate-200 hover:text-[#007AFF] hover:bg-[#F5F5F7]'
                            }`}
