...
const AIInsights: React.FC<Props> = ({ holdings, refreshNonce = 0 }) => {
  ...
  // One auto-generation per mount, at most. Without this guard a failing
  // generate would be retried on every re-render, billing in a loop.
  const autoGenerated = useRef(false);

  // Cache read on mount — free and instant. If the backend reports the cached
  // text no longer matches the portfolio, generate once.
  //
  // The trigger is *data changed*, not *page loaded*: an effect keyed on
  // `holdings` would bill once per visitor per load, which is what the manual
  // button originally existed to prevent. Keyed on staleness, the first viewer
  // after a change pays and everyone after reads the refreshed cache, so cost
  // tracks how often the portfolio changes rather than how many people look at
  // it.
  useEffect(() => {
    let cancelled = false;
    fetchInsightsCached()
      .then(data => {
        ...
        // Nothing cached and no holdings yet is not staleness — it is an empty
        // portfolio, and generating for it would only produce an error.
        if (data.stale && holdings.length > 0 && !autoGenerated.current) {
