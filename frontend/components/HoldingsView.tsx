...
const HoldingsView: React.FC<Props> = ({
  ...
  const LotSortIndicator = ({ col }: { col: typeof lotSortKey }) => (
    <svg className={`w-2.5 h-2.5 ml-0.5 ${lotSortKey === col ? 'text-[#1A3A6B]' : 'opacity-20'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-wrap items-center gap-6">
                  ...
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {allBrokerages.map(b => (
                      <label key={b} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#1A3A6B] focus:ring-[#1A3A6B]" checked={selectedBrokerages.includes(b)} onChange={() => toggleBrokerage(b)} />
                  ...
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {allTickers.map(t => (
                      <label key={t} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#1A3A6B] focus:ring-[#1A3A6B]" checked={selectedTickers.includes(t)} onChange={() => toggleTickerFilter(t)} />
          ...
          <div className="relative shrink-0">
            <div className="absolute -top-2 left-2 bg-white px-1 z-10 flex items-center gap-1">
              <span className="text-[9px] font-black text-[#1A3A6B] uppercase tracking-tighter">After-Tax Keep</span>
            ...
            <div className="flex items-center gap-2 pl-3 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl">
              <input type="range" min={0} max={100} step={5} value={keepPct} onChange={e => setKeepPct(Number(e.target.value))} className="w-28 accent-[#1A3A6B]00 cursor-pointer" />
              <span className="text-sm font-bold text-[#1A3A6B] w-8 text-right">{keepPct}%</span>
            </div>
          </div>

          <button onClick={resetFilters} className="text-xs font-bold text-slate-400 hover:text-[#1A3A6B] transition-colors uppercase tracking-widest px-2 shrink-0">
            Reset
          </button>
        </div>
      </div>
              ...
              <tr className="bg-[#F5F5F7] border-b border-[#D2D2D7]">
                <th className="px-4 py-4 w-8">
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center rounded-lg text-slate-400 hover:text-[#1A3A6B] hover:bg-[#F5F5F7] transition-all"
                ...
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B]" onClick={() => handleSort('ticker')}>
                  <div className="flex items-center">Ticker <SortIndicator column="ticker" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B] text-right" onClick={() => handleSort('quantity')}>
                ...
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B] text-right" onClick={() => handleSort('totalCost')}>
                  <div className="flex items-center justify-end">Total Cost <SortIndicator column="totalCost" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B] text-right" onClick={() => handleSort('currentPrice')}>
                  <div className="flex items-center justify-end">Current Price <SortIndicator column="currentPrice" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B] text-right" onClick={() => handleSort('marketValue')}>
                  <div className="flex items-center justify-end">Market Value <SortIndicator column="marketValue" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#1A3A6B] text-right" onClick={() => handleSort('gain')}>
                    ...
                    <tr className={`hover:bg-[#F5F5F7] transition-colors group border-t border-[#D2D2D7] ${isExpanded ? 'bg-slate-50/70' : ''}`}>
                      ...
                      <td className="px-4 py-4 text-right">
                        {analystMedian[row.ticker] ? (() => {
                          ...
                          return (
                            <div>
                              <div className="text-sm font-bold text-[#1A3A6B]">${analystMedian[row.ticker].toFixed(2)}</div>
                    ...
                    {isExpanded && row.brokerageGroups.map(bg => {
                      ...
                      return (
                        <React.Fragment key={brokerageKey}>
                          ...
                          <tr className={`border-t border-[#D2D2D7] ${isBrokerageExpanded ? 'bg-[#1A3A6B]/5' : 'bg-[#F5F5F7]'} hover:bg-[#1A3A6B]/5 transition-colors`}>
                            <td className="pl-8 pr-4 py-3">
                              <button
                                onClick={() => toggleBrokerageRow(brokerageKey)}
                                className="w-5 h-5 flex items-center justify-center rounded-md text-[#D2D2D7] hover:text-[#1A3A6B] hover:bg-[#E5EDF7] transition-all"
                              >
                            ...
                            <td className="px-4 py-3">
                              <span className={`inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tight ${
                                bg.brokerage.toLowerCase().includes('robinhood') ? 'bg-[#1A3A6B] text-white' :
                            ...
                            <td className="px-4 py-3 text-right">
                              {analystMedian[row.ticker] ? (() => {
                                ...
                                return (
                                  <div>
                                    <div className="text-[11px] font-bold text-[#1A3A6B]">${target.toFixed(2)}</div>
                          ...
                          {isBrokerageExpanded && (() => {
                            const th = (col: typeof lotSortKey, label: string) => (
                              <td className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest whitespace-nowrap text-right cursor-pointer select-none hover:text-[#1A3A6B] transition-colors"
                                  style={{ color: lotSortKey === col ? '#1A3A6B' : undefined }}
                                  onClick={() => handleLotSort(col)}>
                                <div className="flex items-center justify-end gap-0.5">{label}<LotSortIndicator col={col} /></div>
                              </td>
                            );
                            return (
                              <tr className="bg-[#E5EDF7]/30 border-t border-[#D2D2D7]/60">
                                <td></td>
                                <td className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest whitespace-nowrap cursor-pointer select-none hover:text-[#1A3A6B] transition-colors"
                                    style={{ color: lotSortKey === 'buyDate' ? '#1A3A6B' : undefined }}
                            ...
                            return (
                              <tr key={`${brokerageKey}-lot-${idx}`} className="bg-white border-t border-[#D2D2D7]/40">
                                ...
                                <td className="px-4 py-2 text-right">
                                  <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${lot.isLongTerm ? 'bg-[#E5EDF7] text-[#122C52]' : 'bg-[#E5E5EA] text-[#6E6E73]'}`}>
                                ...
                                <td className="px-4 py-2 text-right">
                                  {analystMedian[row.ticker] ? (
                                    <div className="text-[11px] font-bold text-[#1A3A6B]">${analystMedian[row.ticker].toFixed(2)}</div>
                                  ) : <div className="text-[10px] text-slate-300">—</div>}
                                </td>
                                <td className="px-4 py-2 text-right">
                                  {analystMedian[row.ticker] ? (() => {
