...
const HoldingsView: React.FC<Props> = ({
  ...
  const LotSortIndicator = ({ col }: { col: typeof lotSortKey }) => (
    <svg className={`w-2.5 h-2.5 ml-0.5 ${lotSortKey === col ? 'text-[#0052FF]' : 'opacity-20'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filter Bar */}
      <div className="bg-white p-5 rounded-lg border border-[#E5E5E5] relative z-30">
        <div className="flex flex-wrap items-center gap-6">
                  ...
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {allBrokerages.map(b => (
                      <label key={b} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#0052FF] focus:ring-[#0052FF]" checked={selectedBrokerages.includes(b)} onChange={() => toggleBrokerage(b)} />
                  ...
                  <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                    {allTickers.map(t => (
                      <label key={t} className="flex items-center px-3 py-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#0052FF] focus:ring-[#0052FF]" checked={selectedTickers.includes(t)} onChange={() => toggleTickerFilter(t)} />
          ...
          <div className="relative shrink-0">
            <div className="absolute -top-2 left-2 bg-white px-1 z-10 flex items-center gap-1">
              <span className="text-[9px] font-black text-[#0052FF] uppercase tracking-tighter">After-Tax Keep</span>
            ...
            <div className="flex items-center gap-2 pl-3 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl">
              <input type="range" min={0} max={100} step={5} value={keepPct} onChange={e => setKeepPct(Number(e.target.value))} className="w-28 accent-[#0052FF]00 cursor-pointer" />
              <span className="text-sm font-bold text-[#0052FF] w-8 text-right">{keepPct}%</span>
            </div>
          </div>

          <button onClick={resetFilters} className="text-xs font-bold text-slate-400 hover:text-[#0052FF] transition-colors uppercase tracking-widest px-2 shrink-0">
            Reset
          </button>
        </div>
      </div>
      ...
      {tickerRows.length === 0 ? (
        <div className="py-20 text-center bg-white border border-[#E5E5E5] rounded-lg">
          <div className="max-w-xs mx-auto space-y-4">
            ...
            <button onClick={resetFilters} className="text-[#0052FF] text-sm font-bold hover:underline">Clear all filters</button>
          </div>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-[#E5E5E5] bg-white overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#F3F3F5] border-b border-[#E5E5E5]">
                ...
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF]" onClick={() => handleSort('ticker')}>
                  <div className="flex items-center">Ticker <SortIndicator column="ticker" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF] text-right" onClick={() => handleSort('quantity')}>
                ...
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF] text-right" onClick={() => handleSort('totalCost')}>
                  <div className="flex items-center justify-end">Total Cost <SortIndicator column="totalCost" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF] text-right" onClick={() => handleSort('currentPrice')}>
                  <div className="flex items-center justify-end">Current Price <SortIndicator column="currentPrice" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF] text-right" onClick={() => handleSort('marketValue')}>
                  <div className="flex items-center justify-end">Market Value <SortIndicator column="marketValue" /></div>
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap cursor-pointer hover:text-[#0052FF] text-right" onClick={() => handleSort('gain')}>
              ...
              {tickerRows.map(row => {
                ...
                return (
                  <React.Fragment key={row.ticker}>
                    ...
                    {/* Level 1 — Ticker */}
                    <tr className={`hover:bg-[#F9F9FB] transition-colors group border-t border-[#E5E5E5] ${isExpanded ? 'bg-slate-50/70' : ''}`}>
                      ...
                      <td className="px-4 py-4">
                        <span className="inline-flex items-center px-2 py-1 bg-[#1A1C1D] text-white rounded text-[10px] font-bold uppercase tracking-tight">
                          {row.ticker}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4 text-right">
                        {analystMedian[row.ticker] ? (() => {
                          ...
                          return (
                            <div>
                              <div className="text-sm font-bold text-[#0052FF]">${analystMedian[row.ticker].toFixed(2)}</div>
                    ...
                    {isExpanded && row.brokerageGroups.map(bg => {
                      ...
                      return (
                        <React.Fragment key={brokerageKey}>
                          ...
                          <tr className={`border-t border-[#E5E5E5] ${isBrokerageExpanded ? 'bg-[#0052FF]/5' : 'bg-[#F9F9FB]'} hover:bg-[#0052FF]/5 transition-colors`}>
                            ...
                            <td className="px-4 py-3 text-right">
                              {analystMedian[row.ticker] ? (() => {
                                ...
                                return (
                                  <div>
                                    <div className="text-[11px] font-bold text-[#0052FF]">${target.toFixed(2)}</div>
                          ...
                          {isBrokerageExpanded && (() => {
                            const th = (col: typeof lotSortKey, label: string) => (
                              <td className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest whitespace-nowrap text-right cursor-pointer select-none hover:text-[#0052FF] transition-colors"
                                  style={{ color: lotSortKey === col ? '#0052FF' : undefined }}
                            ...
                            return (
                              <tr className="bg-[#DDE1FF]/30 border-t border-[#C3C5D9]/60">
                                <td></td>
                                <td className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest whitespace-nowrap cursor-pointer select-none hover:text-[#0052FF] transition-colors"
                                    style={{ color: lotSortKey === 'buyDate' ? '#0052FF' : undefined }}
                                ...
                                {th('gain', 'Unrealized')}
                                <td className="px-4 py-1.5 text-[9px] font-black text-[#737688] uppercase tracking-widest whitespace-nowrap text-right">Term</td>
                                <td></td>
                                <td className="px-4 py-1.5 text-[9px] font-black text-[#737688] uppercase tracking-widest whitespace-nowrap text-right">Target</td>
                                <td className="px-4 py-1.5 text-[9px] font-black text-[#737688] uppercase tracking-widest whitespace-nowrap text-right">Gain to Sell</td>
                              </tr>
                            );
                          })()}
                          ...
                          {isBrokerageExpanded && bg.lots.map((lot: UnrealizedLot, idx: number) => {
                            ...
                            return (
                              <tr key={`${brokerageKey}-lot-${idx}`} className="bg-white border-t border-[#E5E5E5]/40">
                                ...
                                <td className="px-4 py-2 text-right">
                                  <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${lot.isLongTerm ? 'bg-[#DDE1FF] text-[#003EC7]' : 'bg-[#E2E2E4] text-[#434656]'}`}>
                                ...
                                <td className="px-4 py-2 text-right">
                                  {analystMedian[row.ticker] ? (
                                    <div className="text-[11px] font-bold text-[#0052FF]">${analystMedian[row.ticker].toFixed(2)}</div>
                                  ) : <div className="text-[10px] text-slate-300">—</div>}
                                </td>
                                <td className="px-4 py-2 text-right">
                                  {analystMedian[row.ticker] ? (() => {
