...
from backend.app.db.schema import Transaction as DBTransaction, RealizedGain
...
def process_transactions(db: Session, brokerage_name: str = None):
    ...
    if brokerage_name:
        transactions = db.query(DBTransaction).filter(DBTransaction.brokerage == brokerage_name).order_by(DBTransaction.date).all()
    else:
